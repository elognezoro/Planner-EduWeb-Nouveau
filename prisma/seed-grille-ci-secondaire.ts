 ...
 *    « QZ » (quinzaine) porté comme une séance hebdomadaire (la plateforme ne gère pas le
 *    bihebdomadaire) ; la demi-classe est gérée hors grille.
 *  - Matières facultatives (« fac ») EXCLUES (LV2 en 1èreC/D et TleC/D).
...
const SCIENCES: Record<string, (number[] | null)[]> = {
  ...
  "Physique-Chimie": [[83], [83], [83], [110], [110, 83], [55, 110, 110], [55, 83], [55, 138, 110], [55, 110, 83], null, [110, 110, 110], [55, 110, 110]],
  "SVT":             [[83], [83], [83], [110], [83],      [110],          [55, 83], [110],           [55, 110],     [110], [110],          [55, 55, 165]],
};
...
async function main() {
  ...
  const nomsDisciplines = [...Object.keys(REGULIERES), ...Object.keys(SCIENCES)];
  ...
  type Ligne = { niveauId: string; disciplineId: string; seancesMinutes: number[]; nbSeances: number; heuresHebdo: number };
  const lignes: Ligne[] = [];
  const ajouter = (discNom: string, niveauIdx: number, seances: number[]) => {
    lignes.push({
      ...
      heuresHebdo: seances.reduce((a, b) => a + b, 0) / 60,
    });
  };
  ...
  console.log(`\n✓ Grille nationale CI : ${supprimees} ligne(s) remplacée(s) par ${lignes.length} ligne(s) (circulaire N° 0311).`);
  ...
  for (const n of NIVEAUX) {
    ...
    console.log(`   ${n.padEnd(7)} : ${(min / 60).toFixed(2)} h/sem élève (${min} min)`);
  }
