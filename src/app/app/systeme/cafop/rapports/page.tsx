...
import { Card, StatCard, Badge } from "@/components/app/ui";
import { EnteteCafop } from "../entete-cafop";

export async function generateMetadata(): Promise<Metadata> {
  return { title: appliquerTerme("CAFOP — Rapports", await termeCafopCourant()) };
}
...
export default async function RapportsCafopTabPage() {
  ...
  // Périmètre : cafop_admin → son centre ; sinon tous les CAFOP du pays consulté.
  const wherePromo =
    u.roleReel === "cafop_admin"
      ? { type: "cafop_promotion" as const, cafopId: u.portee.cafopId ?? "__aucune__" }
      : { type: "cafop_promotion" as const, cafop: { pays } };

  const [promotions, nbCentres, regions] = await Promise.all([
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <Card>
        <h2 className="mb-3 font-display text-base font-bold text-forest-900">Promotions d&apos;élèves-maîtres</h2>
        {lignes.length === 0 ? (
