...
/** Inscrit nominativement un utilisateur à un cours SOUS un rôle donné (admin). Idempotent. */
export async function inscrireRole(coursId: string, utilisateurId: string, roleCible: string): Promise<EtatInscription> {
  const g = await gardeAdmin();
  if (!g.ok) return g;
  if (!coursId || !utilisateurId) return { ok: false, message: "Paramètres manquants." };
  const role = roleCible && estRoleValide(roleCible) ? roleCible : null;
  ...
  try {
    await prisma.inscriptionCours.upsert({
      ...
      create: { utilisateurId, coursId, source: "nominative", roleCible: role },
      // Déjà inscrit : on (re)pose le rôle sous lequel on l'inscrit, sans toucher progression/source.
      update: { roleCible: role, derniereActivite: new Date() },
    });
    revalidatePath(PAGE);
  } catch (e) {
    console.error("[inscriptions-role] inscrire :", e);
...
export async function desinscrireRole(coursId: string, utilisateurId: string): Promise<EtatInscription> {
  const g = await gardeAdmin();
  if (!g.ok) return g;
  if (!coursId || !utilisateurId) return { ok: false, message: "Paramètres manquants." };
  try {
    ...
    console.error("[inscriptions-role] désinscrire :", e);
    return { ok: false, message: "Erreur technique." };
  }
