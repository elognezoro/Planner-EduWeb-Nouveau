...
export function FormCours({ opts, cours }: { opts: OptionsCommunes; cours?: { id: string; titre: string; description: string | null; categorieId: string | null; niveau: string | null; publicCible: string[]; dureeMinutes: number | null; dateFormation: string | null; dateLimiteInscription: string | null; seuilCompletion: number; progressionSequentielle: boolean; estGuide: boolean; estSeminaire: boolean; modulesGroupes: boolean; attestationSignataire: string | null; attestationFonction: string | null; attestationMention: string | null } }) {
  ...
  return (
    <form action={action} className="space-y-3 rounded-2xl border border-cream-200 bg-cream-50/40 p-4">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <div><label className={label}>Fin des inscriptions <span className="font-normal text-ink-700/50">(facultatif — ferme les liens)</span></label><input name="dateLimiteInscription" type="datetime-local" defaultValue={cours?.dateLimiteInscription ?? ""} className={champ} /></div>
      </div>
