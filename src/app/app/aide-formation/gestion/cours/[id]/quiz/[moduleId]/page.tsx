...
export default async function QuizEditeurPage({ params }: { params: Promise<{ id: string; moduleId: string }> }) {
  ...
  const lecon = await prisma.moduleCours.findFirst({
    ...
    select: {
      titre: true,
      quiz: { select: { id: true, consigne: true, seuilReussite: true, questions: { orderBy: { ordre: "asc" }, select: { id: true, enonce: true, type: true, points: true, choix: { orderBy: { ordre: "asc" }, select: { texte: true, correct: true } } } } } },
    },
  });
  ...
  const quizBase = lecon.quiz ?? (await prisma.quiz.create({ data: { moduleId }, select: { id: true, consigne: true, seuilReussite: true } }));
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <section className="space-y-2">
        ...
        <FormReglages moduleId={moduleId} coursId={coursId} seuil={quizBase.seuilReussite} consigne={quizBase.consigne} />
      </section>
              ...
              <Card key={q.id} className="py-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-1">
                    <FormQuestion quizId={quizBase.id} question={{ id: q.id, enonce: q.enonce, type: q.type, points: q.points, choix: q.choix }} />
