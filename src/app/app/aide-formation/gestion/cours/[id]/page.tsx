...
export default async function EditionCoursPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [cours, categories] = await Promise.all([
    prisma.cours.findUnique({
      where: { id },
      select: { id: true, titre: true, description: true, categorieId: true, niveau: true, publicCible: true, dureeMinutes: true, dateFormation: true, dateLimiteInscription: true, statut: true, imageUrl: true,
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <section className="space-y-2">
        ...
        <FormCours opts={opts} cours={{ ...cours, dateFormation: cours.dateFormation ? cours.dateFormation.toISOString().slice(0, 16) : null, dateLimiteInscription: cours.dateLimiteInscription ? cours.dateLimiteInscription.toISOString().slice(0, 16) : null }} />
