...
import { genererLivretDocx, type CoursDoc } from "@/lib/lms-livret-docx";
...
/**
 * Export Word (.docx) PAGINÉ du livret d'un cours — Word gère la pagination et les numéros de page.
 * Deux versions : apprenant (défaut) et formateur (?corrige=1, réservé admin/tuteur). Un apprenant
 * qui demande la version corrigée obtient la version sans réponses (jamais les corrigés).
 */
export async function GET(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  // Corrigés réservés aux formateurs : un apprenant reçoit la version sans réponses.
  const corrige = corrigeParam && estTuteur;

  // Logo officiel : récupéré depuis l'asset statique de la même origine (fiable en serverless).
  ...
  const buffer = await genererLivretDocx({ cours: cours as CoursDoc, corrige, logo });
  const nom = `livret-${slug}-${corrige ? "formateur" : "apprenant"}.docx`;
